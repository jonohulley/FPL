	...
	class Program
	{
		...
		public static void writeEventsToDB()
		{
			if (File.Exists("OTSPrimary.UDL"))
			{
				...
				try
				{
					...
					/*if(GWStats  != null)
					{
						for (int i = 0; i < GWStats.Count; i++)
						{
							string CommandText = getQueryString(FPL.LoadType.STATS, i);
							OleDbCommand updatePlayers = new OleDbCommand();
							updatePlayers.Connection = primaryDatabaseConnection;
							updatePlayers.CommandText = CommandText;
							int NumRows = updatePlayers.ExecuteNonQuery();
		...
		public static string getEventsString (int i)
		{
			string eventstring = string.Format("BEGIN TRANSACTION;" +
									"UPDATE  [dbo].[Events] WITH(UPDLOCK, SERIALIZABLE)" + "" +
										"SET [name] = '{1}'" +
										",[deadline_time] = '{2}'" +
										",[average_entry_score] = {3}" +
										",[finished] = {4}" +
										",[data_checked] = {5}" +
										",[highest_scoring_entry] = {6}" +
										",[deadline_time_epoch] = {7}" +
										",[deadline_time_game_offset] = {8}" +
										",[highest_score] = {9}" +
										",[is_previous] = {10}" +
										",[is_current] = {11}" +
										",[is_next] = {12}" +
										...
										",[freehit_played] = {16}" +
										",[most_selected] = '{17}'" +
										",[most_transferred_in] = '{18}'" +
										",[top_player] = '{19}'" +
										",[top_player_points] = {20}" +
										",[transfers_made] = {21}" +
										",[most_captained] = '{22}'" +
										",[most_vice_captained] = '{23}'" +
										"WHERE [ID] = {0}; " +
									"IF @@ROWCOUNT = 0 " +
									"BEGIN " +
									"INSERT [dbo].[Events] ([ID],[name],[deadline_time],[average_entry_score],[finished],[data_checked],[highest_scoring_entry]" +
									...
									"END " +
									"COMMIT TRANSACTION; ", listfull.events[i].id, listfull.events[i].name, listfull.events[i].deadline_time, listfull.events[i].average_entry_score, Convert.ToByte(listfull.events[i].finished),
									Convert.ToByte(listfull.events[i].data_checked), listfull.events[i].highest_scoring_entry == null ? 0 : listfull.events[i].highest_scoring_entry, listfull.events[i].deadline_time_epoch, listfull.events[i].deadline_time_game_offset,
									listfull.events[i].highest_score == null ? 0 : listfull.events[i].highest_score, Convert.ToByte(listfull.events[i].is_previous), Convert.ToByte(listfull.events[i].is_current), Convert.ToByte(listfull.events[i].is_next), listfull.events[i].bboost, listfull.events[i].tripCap,
									listfull.events[i].wildcard, listfull.events[i].freehit, listfull.events[i].most_selected_name == null ? "" : listfull.events[i].most_selected_name, listfull.events[i].most_transferred_in_name == null ? "" : listfull.events[i].most_transferred_in_name, listfull.events[i].top_element_name,
									listfull.events[i].top_element_points, listfull.events[i].transfers_made, listfull.events[i].most_captained_name == null ? "" : listfull.events[i].most_captained_name, listfull.events[i].most_vice_captained_name == null ? "" : listfull.events[i].most_vice_captained_name);
		...
		public static string getElementsString (int i)
		{
			string elementstring = string.Format("BEGIN TRANSACTION;" +
									"UPDATE  [dbo].[Players] WITH(UPDLOCK, SERIALIZABLE)" + "" +
										...
										",[crners_fkicks_order] = {55}" +
										",[crners_fkicks_note] = '{56}'" +
										",[fkicks_order] = {57}" +
										",[fkicks_note] = '{58}'" +
										",[pens_order] = {59}" +
										",[pens_note] = '{60}'" +
									...
									"END " +
									"COMMIT TRANSACTION; ", listfull.elements[i].id //0
									, listfull.elements[i].web_name.Replace("'", "''")
									, listfull.elements[i].first_name.Replace("'", "''")
									, listfull.elements[i].second_name.Replace("'", "''")
									, listfull.elements[i].cost_change_start_fall
									, listfull.elements[i].chance_of_playing_next_round == null ? 100 : listfull.elements[i].chance_of_playing_next_round//5
									, listfull.elements[i].chance_of_playing_this_round == null ? 100 : listfull.elements[i].chance_of_playing_this_round
									, listfull.elements[i].code
									, listfull.elements[i].cost_change_event
									, listfull.elements[i].cost_change_event_fall
									, listfull.elements[i].dreamteam_count //10
									, listfull.elements[i].position
									, listfull.elements[i].ep_next
									, listfull.elements[i].ep_this
									, listfull.elements[i].event_points
									, listfull.elements[i].form //15
									, Convert.ToByte(listfull.elements[i].in_dreamteam)
									, "test"
									, listfull.elements[i].news_added
									, listfull.elements[i].now_cost
									, listfull.elements[i].points_per_game //20
									, listfull.elements[i].selected_by_percent
									, Convert.ToByte(listfull.elements[i].special)
									, listfull.elements[i].squad_number == null ? 0 : listfull.elements[i].squad_number
									, listfull.elements[i].status
									, listfull.elements[i].team_name //25
									, listfull.elements[i].team_code
									, listfull.elements[i].total_points
									, listfull.elements[i].transfers_in
									, listfull.elements[i].transfers_in_event
									, listfull.elements[i].transfers_out //30
									, listfull.elements[i].transfers_out_event
									, listfull.elements[i].value_form
									, listfull.elements[i].value_season
									, listfull.elements[i].minutes
									, listfull.elements[i].goals_scored //35
									, listfull.elements[i].assists
									, listfull.elements[i].clean_sheets
									, listfull.elements[i].goals_conceded
									, listfull.elements[i].own_goals
									, listfull.elements[i].penalties_safed //40
									, listfull.elements[i].penalties_missed
									, listfull.elements[i].yellow_cards
									, listfull.elements[i].red_cards
									, listfull.elements[i].saves
									, listfull.elements[i].bonus //45
									, listfull.elements[i].bps
									, listfull.elements[i].influence
									, listfull.elements[i].creativity
									, listfull.elements[i].threat
									, listfull.elements[i].ict_index //50
									, listfull.elements[i].influence_rank
									, listfull.elements[i].influence_rank_type
									, listfull.elements[i].creativity_rank
									, listfull.elements[i].creativity_rank_type
									, listfull.elements[i].corners_and_indirect_freekicks_order == null ? 100 : listfull.elements[i].corners_and_indirect_freekicks_order //55
									, listfull.elements[i].corners_and_indirect_freekicks_text
									, listfull.elements[i].direct_freekicks_order == null ? 100 : listfull.elements[i].direct_freekicks_order
									, listfull.elements[i].direct_freekicks_text
									, listfull.elements[i].penalties_order == null ? 100 : listfull.elements[i].penalties_order
									, listfull.elements[i].penalties_text //60
									, listfull.elements[i].cost_change_start);
		...
		public static string getDFCString(int i)
		{
			string DFCString = string.Format("BEGIN TRANSACTION;" +
									...
									"IF @@ROWCOUNT = 0 " +
									"BEGIN " +
									"INSERT INTO [dbo].[DashForCashRank]" +
		...
		public static string getQueryString(FPL.LoadType type, int i)
		{
			string queryString = "";
			switch (type)
			{
